...
while cam_item.setParam('isOpen') == '1':
	if RDK.getParam('active_cap') == 'True':
		#TAKE A FRAME
		bgr_image = None
		...
		if cv.countNonZero(blue_mask) > 10:
			RDK.setParam('color', 'blue')
		if cv.countNonZero(green_mask) > 10:
			RDK.setParam('color', 'green')
		if cv.countNonZero(red_mask) > 10:
			RDK.setParam('color', 'red')
		if cv.countNonZero(white_mask) > 10:
			RDK.setParam('color', 'white')
		if cv.countNonZero(yellow_mask) > 10:
			RDK.setParam('color', 'yellow')
		if cv.countNonZero(orange_mask) > 10:
		...
		if contours:
			...
			x_mm = (int(x) - (W_PIXELS / 2)) * PIXEL_SIZE
			y_mm = (int(y) - (H_PIXELS / 2)) * PIXEL_SIZE
